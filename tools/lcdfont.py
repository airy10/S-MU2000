...
ものを `art/lcdfont.txt` に書けば、エミュレータがそちらを上から被せる。
  ...
  python tools/lcdfont.py sheet              未定義のコードを空の枠で並べる
...
def cmd_show(a):
    ...
    side_by_side([("%02x" % c, art[c]) for c in cs if c in art])
    if miss:
...
def main():
    ...
    p = sub.add_parser("show"); p.add_argument("codes", nargs="*"); p.set_defaults(fn=cmd_show)
